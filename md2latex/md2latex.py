...
class MarkdownToLatex:
	...
	def markdownFile(self, file):
		with open(file) as f:
			md = f.read()
		md = self.markdownify(md)
		return md
	
	def markdownWithTemplate(self, markdown_file, template):
		with open(template) as f:
			temp = f.read()
		md = self.markdownFile(markdown_file)
		rep = r'%BODY%'
		print(rep)
		temp = temp.replace(rep, md)
		return temp
...
if __name__ == "__main__":
	...
	if args['template']:
		if not os.path.exists(args['template']):
			...
			exit()
		result = MarkdownToLatex().markdownWithTemplate(args['input_file'], args['template'])
	else:
		result = MarkdownToLatex().markdownFile(args['input_file'])
